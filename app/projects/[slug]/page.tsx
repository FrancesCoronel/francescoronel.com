...
export default async function ProjectPage({ params }: PageProps) {
  ...
  return (
    <>
      {/* Hero */}
      <section className="border-b border-horchata-200 bg-horchata-50 py-16 md:py-20 dark:border-navy-700 dark:bg-navy-900">
        <div className="mx-auto max-w-[var(--container-max)] px-6">
          <Link
            href="/projects"
            className="mb-8 inline-flex items-center gap-1.5 text-sm text-navy-500 transition-colors hover:text-navy-800 dark:text-white/50 dark:hover:text-white"
          >
          ...
          <div className="flex flex-col gap-8 md:flex-row md:items-start md:gap-12">
            ...
            {project.logo ? (
              <div className="flex-shrink-0">
                <Image
                  ...
                  width={96}
                  height={96}
                  className="h-20 w-20 rounded-2xl object-contain md:h-24 md:w-24"
                />
              </div>
            ) : (
              <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-2xl bg-horchata-100 text-4xl dark:bg-navy-700 md:h-24 md:w-24">
                🛠️
              </div>
            )}
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${CATEGORY_COLORS[project.category] ?? CATEGORY_COLORS["side-project"]}`}>
              ...
              <h1 className="mt-3 text-4xl font-bold text-navy-900 dark:text-horchata-100 md:text-5xl">
                {project.title}
              </h1>
              <p className="mt-2 text-lg text-horchata-700 dark:text-horchata-400">
                {project.tagline}
              </p>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                {project.github && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 rounded-full border border-horchata-200 bg-white px-5 py-3 text-sm font-medium text-navy-700 transition-colors hover:border-horchata-400 hover:text-navy-900 dark:border-navy-600 dark:bg-navy-800 dark:text-horchata-200 dark:hover:border-navy-400"
                  >
                    ...
                    {stars != null && (
                      <>
                        ...
                        <span className="flex items-center gap-1">
                          <svg className="h-3.5 w-3.5 fill-current text-horchata-500" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                ...
                {project.organization && (
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full border border-horchata-200 bg-white px-5 py-3 text-sm font-medium text-navy-700 transition-colors hover:border-horchata-400 hover:text-navy-900 dark:border-navy-600 dark:bg-navy-800 dark:text-horchata-200 dark:hover:border-navy-400"
                  >
                    View organization →
                  </Link>
                )}
        ...
        <div className="mx-auto max-w-[var(--container-max)] px-6">
          <div className="grid gap-12 md:grid-cols-3">
            ...
            <div className="md:col-span-2 space-y-10">
              {/* Description */}
              <div>
                <p className="text-sm font-bold uppercase tracking-widest text-horchata-700">Overview</p>
              ...
              {project.highlights.length > 0 && (
                <div>
                  <p className="text-sm font-bold uppercase tracking-widest text-horchata-700">Highlights</p>
                  <ul className="mt-4 space-y-3">
                    {project.highlights.map((h, i) => (
                      <li key={i} className="flex items-start gap-3 rounded-xl border border-horchata-100 bg-horchata-50 p-4 dark:border-navy-700 dark:bg-navy-800">
                        <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-horchata-700 text-[10px] font-bold text-white dark:bg-horchata-500 dark:text-navy-900">
                          {i + 1}
                        </span>
                        <span className="text-sm text-navy-700 dark:text-horchata-200">{h}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {slug === "nailed-it-tracker" && (
                <div className="space-y-10">
                  <div>
                    <p className="text-sm font-bold uppercase tracking-widest text-horchata-700">Episodes</p>
                  ...
                  <div>
                    <p className="text-sm font-bold uppercase tracking-widest text-horchata-700">Contestants</p>
            ...
            <aside className="space-y-6">
              ...
              {project.skills.length > 0 && (
                <div className="rounded-2xl border border-horchata-200 bg-horchata-50 p-5 dark:border-navy-700 dark:bg-navy-800">
                  <p className="text-xs font-bold uppercase tracking-widest text-horchata-700 dark:text-horchata-500">
                    Skills & Tags
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {project.skills.map((skill) => (
                      <span
                        key={skill}
                        className="rounded-full border border-horchata-200 bg-white px-3 py-1 text-xs font-medium text-navy-700 dark:border-navy-600 dark:bg-navy-700 dark:text-horchata-200"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              {project.github && (
                <div className="rounded-2xl border border-horchata-200 bg-horchata-50 p-5 dark:border-navy-700 dark:bg-navy-800">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-3 flex items-center gap-2 text-sm font-medium text-navy-700 transition-colors hover:text-horchata-700 dark:text-horchata-200 dark:hover:text-horchata-400"
                  >
                    <svg className="h-4 w-4 flex-shrink-0 fill-current" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                    <span className="font-mono">{project.github}</span>
                    <svg className="ml-auto h-3.5 w-3.5 flex-shrink-0 opacity-40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 17L17 7M17 7H7M17 7v10"/></svg>
                  </a>
                  {stars != null && (
                    <div className="mt-3 flex items-center gap-1.5 text-sm">
                      ...
                      <span className="font-bold text-navy-900 dark:text-horchata-100">{stars.toLocaleString()}</span>
                      <span className="text-navy-500 dark:text-white/50">stars</span>
                    </div>
                  )}
                </div>
              )}

...
const socialLinks: { label: string; href: string; rel?: string; icon: React.ReactNode }[] = [
  {
    label: "LinkedIn",
  ...
  {
    label: "Bluesky",
    href: "https://bsky.app/profile/francescoronel.bsky.social",
    rel: "me",
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 568 501">
...
export function Footer() {
  return (
    <footer className="border-t border-horchata-200 bg-horchata-100 dark:border-navy-800 dark:bg-navy-950">
      <div className="mx-auto max-w-[var(--container-max)] px-6 pt-12 pb-8 sm:pt-16 lg:pt-20">

        {/* Top: brand (left) + nav columns (right, compact) */}
        <div className="lg:grid lg:grid-cols-[1fr_auto] lg:gap-16">
          <div>
            <Link
              href="/"
              className="text-lg font-bold text-navy-900 transition-colors hover:text-navy-700 dark:text-white dark:hover:text-white/80"
            >
              Hi, I&apos;m Frances! 👋🏽
            </Link>
          ...
          {/* Nav: stacks on mobile, 3 cols at sm, individual grid items at lg */}
          <nav
            aria-label="Footer navigation"
            className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-3 lg:mt-0"
          >
            {footerNav.map((group) => (
        ...
        {/* Newsletter — full-width section with label+description left, form right */}
        <div className="mt-12 border-t border-horchata-200 pt-8 dark:border-navy-700 sm:mt-16 lg:mt-20 lg:flex lg:items-center lg:justify-between">
          <div>
